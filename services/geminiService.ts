...
export const generateQuiz = async (topic: string, difficulty: Difficulty, questionCount: number): Promise<Question[]> => {
  try {
    ...
    throw new Error("Failed to generate quiz. The AI might be busy or the topic might be too specific. Please try again.");
  }
};

export const getImprovementSuggestions = async (topic: string, incorrectAnswers: UserAnswer[]): Promise<string> => {
  ...
  try {
    const incorrectQuestionsSummary = incorrectAnswers
      .map(answer => `- Question: "${answer.question}" (Your answer: "${answer.selectedAnswer}", Correct: "${answer.correctAnswer}")`)
      .join('\n');
    ...
    throw new Error("Failed to get improvement suggestions. Please try again later.");
  }
};
